...
from satellitepy.data.tools import split_xview_labels
from satellitepy.utils.path_utils import create_folder, init_logger, get_project_folder
import logging

"""
Split original xview label file into one label file for each image.
"""
...
def run(args):
    ...
    # Init logger
    log_path = Path(
        out_folder) / 'split_xview_labels.log' if args.log_path == None else args.log_path
    init_logger(config_path=args.log_config_path, log_path=log_path)
    logger = logging.getLogger(__name__)
    logger.info(
        f'No log path is given, the default log path will be used: {log_path}')
    logger.info('Saving split xview labels')
