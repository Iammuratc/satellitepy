...
def get_args():
    ...
    parser.add_argument('--unzip', type=bool, required=True,
                        help='If datasets with compressed files should be automatically unzipped')
...
def download_VHR_10_dataset_coco(in_path, log_config, log_path=None, unzip=True):
    ...
    try:
        ...
        if unzip:
            try:
                patoolib.extract_archive(file, outdir=in_path)
            except patoolib.util.PatoolError:
                if logging:
                    logging.warning(
                        'Failed to extract .rar file. It is likely that you are missing the non-free software required to handle the archive format.')
            else:
                if logging:
...
if __name__ == '__main__':
    args = get_args()
    download_VHR_10_dataset_coco(args.in_folder, args.log_config, args.log_path, args.unzip)
