...
def random_flip(image, mask=None, boxes=[], crop_center=None):
    """
    image: h x w x c
    ...
    return image, mask, boxes, crop_center
...
def rotation_transform(src_point, degree):
    ...
    rot_matrix = [[np.cos(radian), -np.sin(radian)],
                [np.sin(radian), np.cos(radian)]]
    rot_matrix = np.asarray(rot_matrix, dtype=np.float32)
    rot_pts = np.matmul(rot_matrix, src_point)
    return rot_pts
