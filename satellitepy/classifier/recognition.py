...
class ClassifierRecognition(Classifier):
    ...
    def train(self):
        ...
        loss_func = nn.CrossEntropyLoss(
            weight=self.get_class_weights().to(device))
        optimizer = optim.SGD(model.parameters(), lr=0.001, momentum=0.9)

        instances = '    '.join(
            list(self.settings['dataset']['instance_table'].keys()))
        class_weight = '        '.join(
            [str(weight) for weight in self.settings['training']['class_weight']])
    ...
    def plot_conf_mat(self, dataset_part, plot, save):
        ...
        df_conf_mat = pd.DataFrame(
            confusion_matrix,
            index=instance_list,
            columns=instance_list)
        fig, ax = plt.subplots(1)
        # plt.figure(figsize = (10,7))
        ax.set_title(
            f'Exp no: {exp_no} --- Dataset: {dataset_part}\nRows: Predicted --- Cols: Ground truth')
        ...
        if save:
            fig_path = os.path.join(
                self.settings['experiment']['folder'],
                f'conf_mat_{dataset_part}.png')
    ...
    def plot_images(self, instances, dataset_part, save, plot=True):
        ...
        if save:
            fig_folder = os.path.join(
                self.settings['experiment']['folder'],
                f'{y_true_name}_{y_pred_name}_{dataset_part}')
        ...
        for i_fig in range(fig_count):
            ...
            fig.suptitle(
                f'Ground Truth: {y_true_name} Prediction: {y_pred_name}',
                fontsize=15)
            for ind, img_path in enumerate(
                    image_paths[i_fig * fig_subplots:(i_fig + 1) * fig_subplots]):
