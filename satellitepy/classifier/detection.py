...
class ClassifierDetection:
    ...
    def get_yolo_yaml_files(self):
        ...
        # val:
        # /home/murat/Projects/airplane_detection/DATA/Gaofen/val/patches_512/images
    ...
    def get_patch_folders(self):
        ...
        if split_ratio:
            ...
            # Sort image paths, because there are overlaps among some images,
            # check out sequences.json
            ...
            self.get_symlink_folder(
                folder=train_img_symlink_folder,
                paths=split_train_img_paths)
            self.get_symlink_folder(
                folder=test_img_symlink_folder,
                paths=split_test_img_paths)
            self.get_symlink_folder(
                folder=val_img_symlink_folder,
                paths=split_val_img_paths)

            self.get_symlink_folder(
                folder=train_label_symlink_folder,
                paths=split_train_label_paths)
            self.get_symlink_folder(
                folder=test_label_symlink_folder,
                paths=split_test_label_paths)
            self.get_symlink_folder(
                folder=val_label_symlink_folder,
                paths=split_val_label_paths)
    ...
    def get_full_paths(self, folder):
        ...
        sorted_files.sort(key=lambda f: (int(f.split('_')[0]), int(
            f.split('_')[2]), int(f.split('_')[4].split('.')[0])))  # EX: 1_x_0_y_0.json
        ...
        if len(os.listdir(folder)) == len(paths):
            ...
            for path in paths:
                ...
                symlink_path = os.path.join(
                    folder, f"{file_name}_symlink{ext}")
                try:
