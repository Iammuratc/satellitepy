...
class PatchDetection:
    ...
    def get_patches(self, save=False, plot=False):
        ...
        for img_file in img_files:
            ...
            # if label_name in instance_names: # if one of the wanted labels in
            # the instance names, the patches are done

            # Check if any of the label_names match with instance_names
            if any(
                    label_name == instance_name for instance_name in instance_names for label_name in label_names):
                # PATCH COORDINATES IN THE ORIGINAL IMAGE
                y_max, x_max, ch = img.shape[:3]
                patch_y, patch_x = self.get_patch_start_coords(
                    my_max=[y_max, x_max], patch_size=patch_size, overlap=overlap)
                ...
                for y_0 in patch_y:
                    for x_0 in patch_x:
                        ...
                        for i, bbox in enumerate(labels['bboxes']):
                            ...
                            # Skip not interested label names, e.g.,
                            # 'Intersection'
                            ...
                            if box_corner_in_patch >= box_corner_threshold:
                                # INSTANCE NAMES
                                patch_dict['instance_names'].append(
                                    instance_names[i])
                                # shift coords
                                shifted_bbox = np.array(bbox) - [x_0, y_0]
                                # ROTATED BBOXES
                                patch_dict['rotated_bboxes'].append(
                                    shifted_bbox.tolist())
                                # ORTHOGONAL BBOXES
                                rect = geometry.Rectangle(bbox=shifted_bbox)

                                orthogonal_bbox_param = rect.get_orthogonal_bbox_by_limits(
                                    bbox=shifted_bbox, return_params=True)
                                orthogonal_bbox = rect.get_orthogonal_bbox_by_limits(
                                    bbox=shifted_bbox, return_params=False)

                                patch_dict['orthogonal_bbox_params'].append(
                                    orthogonal_bbox_param)
                                patch_dict['orthogonal_bboxes'].append(
                                    orthogonal_bbox)
                        ...
                        if limit_expanded:
                            patch_dict['image'][:y_max - y_0,
                                                :x_max - x_0] = img[y_0:y_max, x_0:x_max]
                        elif y_limit_expanded:
                            patch_dict['image'][:y_max - y_0,
                                                :] = img[y_0:y_max, x_0:x_0 + patch_size]
                        elif x_limit_expanded:
                            patch_dict['image'][:, :x_max -
                                                x_0] = img[y_0:y_0 + patch_size, x_0:x_max]
                        else:
                            patch_dict['image'][:, :] = img[y_0:y_0 +
                                                            patch_size, x_0:x_0 + patch_size]
    ...
    def save_patch_dict(self, patch_dict):
        ...
        label_binary_patch_yolo_folder = self.settings['patch'][
            self.dataset_part]['label_binary_yolo_folder']
        label_patch_dota_folder = self.settings['patch'][self.dataset_part]['label_dota_folder']
        label_binary_patch_dota_folder = self.settings['patch'][
            self.dataset_part]['label_binary_dota_folder']
        ...
        with open(os.path.join(label_patch_dota_folder, f"{patch_name}.txt"), 'w+') as f_0:
            with open(os.path.join(label_binary_patch_dota_folder, f"{patch_name}.txt"), 'w+') as f_1:
                for i, bbox in enumerate(patch_dict['rotated_bboxes']):
                    bbox_ccords = ' '.join(
                        np.array(bbox).flatten().astype(str))
    ...
    def plot_patch_dict(self, patch_dict):
        if patch_dict['instance_names']:
            ...
            ax.imshow(cv2.cvtColor(patch_dict['image'], cv2.COLOR_BGR2RGB))
            for i, rotated_bbox in enumerate(patch_dict['rotated_bboxes']):
                ...
                rect.plot_bbox(
                    bbox=patch_dict['orthogonal_bboxes'][i], ax=ax, c='g')
            plt.show()
    ...
    def get_labels(self, label_path):
        # dota has only text files, so we need to change the code for the
        # labels
        label = {'bboxes': [], 'names': []}
        dataset_name = self.settings['dataset']['name']
        if (dataset_name == 'Gaofen' or dataset_name == 'fair1m'):
            ...
            with open(label_path, 'r') as f:
                bbox_labels = [line[:-1].split(' ')
                               for line in f.readlines()[2:]]
                for bbox_label in bbox_labels:
                    category = bbox_label[-2]  # get label of this box
                    # turning the box coords to floats, but delete the last two
                    # indices because there are the label and the difficulty
                    bbox_label = [[float(i) for i in bbox_label[:-2]]]
